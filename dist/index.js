...
function checkWalletInGitignore() {
    try {
        const gitignorePath = path.join(process.cwd(), '.gitignore');
        if (!fs.existsSync(gitignorePath))
            return false;
        const gitignoreContent = fs.readFileSync(gitignorePath, 'utf8');
        return gitignoreContent.split('\n').some(line => line.trim() === 'wallet.json' || line.trim() === '/wallet.json');
    }
    catch (e) {
        return false;
    }
...
async function getWalletAddress(encodedWallet) {
    try {
...
async function handleWalletEncoding() {
...
async function getBalances(encodedWallet) {
    try {
        const wallet = JSON.parse(Buffer.from(encodedWallet, 'base64').toString());
        const signer = new ArweaveSigner(wallet);
        const turbo = TurboFactory.authenticated({ signer });
        ...
        const tarioBalance = await ario
            .getBalance({ address })
            .then((bal) => new mARIOToken(bal).toARIO());
        return {
